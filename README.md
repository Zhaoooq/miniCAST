...
- 监测：逐路显示气体名称、目标流量、实际流量、偏差、偏差率以及“正常 / 偏高 / 偏低 / 通信异常”状态。监测页没有任何目标流量编辑或设备控制入口。
- 报警：按提示、警告、严重过滤，异常恢复后保留记录。支持导出和带确认的清空。
- 设置：显示设备状态、采样周期、阈值、窗口模式和真实通讯状态；可在普通窗口、无边框窗口和全屏显示之间即时切换，故障、单台超时/离线和恢复操作集中在独立的“模拟测试”页面。
- 运行点：从监测页的“切换”进入。系统预设只读，可选择或复制；客户运行点可仅靠触摸保存、编辑、删除、选择覆盖和确认选择。目标流量只能在此页面编辑，名称自动采用“客户运行点 1～6”，不需要键盘。
